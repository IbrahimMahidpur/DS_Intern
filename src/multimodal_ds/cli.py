...
def run(
    ...
    with console.status(f"Executing graph (Thread: {thread_id})...") as status:
        try:
            ...
            # not the delta. This avoids the manual state-merging problem
            # entirely — no deduplication needed, no dict-hashability issues.
            final_state = initial_state
            for event in graph.stream(initial_state, config=config, stream_mode="values"):
                # event IS the full state after the most recent node ran
                final_state = event

                # Identify which node just ran from the state fields that changed
                # (stream_mode="values" doesn't include node name directly)
                # Use a best-effort heuristic for progress display
                node_hint = ""
                if final_state.get("analysis_tasks") and not initial_state.get("analysis_tasks"):
                    node_hint = "planner"
                elif final_state.get("current_step", 0) > initial_state.get("current_step", 0):
                    node_hint = "executor"
                elif final_state.get("statistical_report") and not initial_state.get("statistical_report"):
                    node_hint = "stats_val"

                if node_hint == "planner":
                    n_tasks = len(final_state.get("analysis_tasks", []))
                    console.print(f"[blue]» Finished node:[/] [bold]planner[/]")
                    if n_tasks:
                        console.print(f"   [green]✔[/] Created plan with {n_tasks} tasks.")
                elif node_hint == "executor":
                    step = final_state.get("current_step", 0)
                    console.print(f"[blue]» Finished node:[/] [bold]executor[/]")
                    console.print(f"   [green]✔[/] Executed task step {step}.")
                elif node_hint == "stats_val":
                    console.print(f"[blue]» Finished node:[/] [bold]stats_val[/]")
                    console.print(f"   [green]✔[/] Statistical validation complete.")

                status.update("Executing graph...")

        except (StopIteration, GeneratorExit):
            # LangGraph raises StopIteration when the graph reaches END — normal termination
            pass
        except RuntimeError as exc:
            if isinstance(exc.__cause__, StopIteration) or "StopIteration" in str(exc):
                pass  # PEP 479 wrapping of StopIteration — normal termination
            else:
                console.print(f"[bold red]Graph execution failed:[/] {exc}")
                import traceback
